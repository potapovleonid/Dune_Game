...
public class Tank extends GameObject implements Poolable {
    ...
    private boolean selectTank;
    private int SIZE_CONTAINER;
    ...
    public void setup(Owner ownerType, float x, float y) {
        this.selectTank = false;
        this.textures = Assets.getInstance().getAtlas().findRegion("tankanim").split(64,64)[0];
        this.selectTexture = Assets.getInstance().getAtlas().findRegion("selecttank");
    ...
    public void update(float dt) {
        if (Gdx.input.isButtonPressed(Input.Buttons.LEFT) && Math.abs(Gdx.input.getX() - position.x) < 32
                && Math.abs(Gdx.input.getX() - position.x) < 32){
            selectTank = true;
        }
        ...
        if (Gdx.input.isButtonPressed(Input.Buttons.RIGHT) && selectTank) {
    ...
    public void updateWeapon(float dt) {
        if (weapon.getType() == Weapon.Type.HARVEST) {
            if (gc.getMap().getResourceCount(this) > 0 && container < SIZE_CONTAINER) {
    ...
    public void render(SpriteBatch batch) {
        if (selectTank){
            batch.draw(selectTexture, position.x - 32, position.y - 32);
        }
        font10.draw(batch, "cont: " + container, position.x - 32, position.y - 35);
